...
FactoryRegistration<FwdModelFactory, ASL2CompartmentModel>
    ASL2CompartmentModel::registration("asl_2comp");

FwdModel *ASL2CompartmentModel::NewInstance()
{
    return new ASL2CompartmentModel();
}
...
void ASL2CompartmentModel::GetParameterDefaults(std::vector<Parameter> &params) const
{
    ...
    int p=0;
    params.push_back(Parameter(p++, "f", 
                               DistParams(m_f_initial, VAR_NONINFORM), 
                               DistParams(m_f_initial, 10),
                               PRIOR_NORMAL, TRANSFORM_LOG()));
    //params.push_back(Parameter(p++, "sig0", DistParams(m_sig0_initial, 1e12), DistParams(m_sig0_initial, 10)));
    if (m_infer_bat) {
        params.push_back(Parameter(p++, "bat", 
                               DistParams(m_bat, VAR_NONINFORM), 
                               DistParams(m_bat, 10),
                               PRIOR_NORMAL, TRANSFORM_LOG()));
    }
    if (m_infer_t1) {
        params.push_back(Parameter(p++, "t1", 
                               DistParams(m_t1, VAR_NONINFORM), 
                               DistParams(m_t1, 10),
                               PRIOR_NORMAL, TRANSFORM_LOG()));
    }
    if (m_infer_t1b) {
        params.push_back(Parameter(p++, "t1b", 
                               DistParams(m_t1b, VAR_NONINFORM), 
                               DistParams(m_t1b, 10),
                               PRIOR_NORMAL, TRANSFORM_LOG()));
    }
    if (m_infer_t1e) {
        params.push_back(Parameter(p++, "t1e", 
                               DistParams(m_t1e, VAR_NONINFORM), 
                               DistParams(m_t1e, 10),
                               PRIOR_NORMAL, TRANSFORM_LOG()));
    }
}
 ...
 */ 
double ASL2CompartmentModel::dM(double t, double f, double bat, double t1, double t1b, double t1e) const
{
...
void ASL2CompartmentModel::EvaluateModel(const ColumnVector &params, ColumnVector &result, const std::string &key) const
{
    int p = 1;
    int nt = data.Nrows();
    if (nt != m_times.size()) {
        throw FabberRunDataError("The number of data volumes was " + stringify(nt) + " but the number of TIs/PLDs specified was " + stringify(m_times.size()));
    }
