...
protected: // Constants
    ...
    int calib_index() const
    {
        return (infertiss ? 2 : 0) + (infertiss ? (infertau ? 1 : 0) : 0)
            + (inferart ? 2 : 0) + (infert1 ? 2 : 0) + (infertaub ? 1 : 0)
            + (inferwm ? (2 + (infertau ? 1 : 0) + (infert1 ? 1 : 0)) : 0)
            + (usepve ? 2 : 0) + 2 + (inferart ? (artdir ? 3 : 4) : 0)
            + (calibon ? 1 : 0);
    }
    ...
    NEWMAT::ColumnVector kcblood_nodisp(const NEWMAT::ColumnVector &tis,
        float deltblood, float taub, float T_1b, float deltll, float T_1ll,
        int n_bolus_total, float delta_bolus,
    ...
    NEWMAT::ColumnVector kctissue_gaussdisp(const NEWMAT::ColumnVector &tis,
        ...
        float sig2, float deltll, float T_1ll, int n_bolus_total,
...
private:
    ...
    static FactoryRegistration<FwdModelFactory, TurboQuasarFwdModel>
        registration;
};
