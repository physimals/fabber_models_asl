/*  fwdmodel_asl_turboquasar.h - Resting state ASL model for TURBO QUASAR
   acquisitions
...
protected: // Constants
    ...
    double t1;
    double t1_MT; // T1 under MT effects
    double t1b;
    double t1wm;
    double t1wm_MT; // T1 under MT effects
    double lambda;
    double slicedt;
    double pretisat;
    ...
    NEWMAT::ColumnVector kcblood_gammadisp(const NEWMAT::ColumnVector &tis, float deltblood,
        ...
        float delta_bolus, const NEWMAT::ColumnVector &bolus_order) const;
    NEWMAT::ColumnVector kcblood_gvf(const NEWMAT::ColumnVector &tis, float deltblood, float taub,
        float T_1b, float s, float p, float deltll, float T_1ll, int n_bolus_total,
        float delta_bolus, const NEWMAT::ColumnVector &bolus_order) const;
    NEWMAT::ColumnVector kcblood_gaussdisp(const NEWMAT::ColumnVector &tis, float deltblood,
        float taub, float T_1b, float sig1, float sig2, float deltll, float T_1ll,
    ...
    NEWMAT::ColumnVector kctissue_gvf(const NEWMAT::ColumnVector &tis, float delttiss, float tau,
        float T1_b, float T1_app, float s, float p, float deltll, float T_1ll, int n_bolus_total,
        float delta_bolus, const NEWMAT::ColumnVector &bolus_order) const;
    NEWMAT::ColumnVector kctissue_gaussdisp(const NEWMAT::ColumnVector &tis, float delttiss,
        float tau, float T_1b, float T_1app, float sig1, float sig2, float deltll, float T_1ll,
        int n_bolus_total, float delta_bolus, const NEWMAT::ColumnVector &bolus_order) const;
